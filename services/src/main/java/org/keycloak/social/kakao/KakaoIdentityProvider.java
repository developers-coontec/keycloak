 ...
 */
public class KakaoIdentityProvider extends
    ...
    public KakaoIdentityProvider(KeycloakSession session, KakaoIdentityProviderConfig config) {
    ...
    protected BrokeredIdentityContext doGetFederatedIdentity(String accessToken) {
        try {
            ...
            if (phoneNumber != null && !phoneNumber.isEmpty()) {
                ...
                user.setUserAttribute(Constants.FIELD_MOBILE_PHONE_NUMBER, phoneNumber);
            }
            ...
            if (birthYear != null && !birthYear.isEmpty() && birthDay != null && !birthDay.isEmpty()) {
                ...
                if (birthYear.length() == 4 && birthDay.length() == 4) {
                    ...
                    user.setUserAttribute(Constants.FIELD_BIRTH_DATE, birthDate);
                }
            }
            String genderCode = getJsonProperty(account, "gender");
            if (genderCode != null && !genderCode.isEmpty()) {
                user.setUserAttribute(Constants.FIELD_GENDER_CODE, genderCode.trim());
            }
            String imageUrl = getJsonProperty(profile, "profile_image_url");
            if (imageUrl != null && !imageUrl.isEmpty()) {
                user.setUserAttribute(Constants.FIELD_PROFILE_IMAGE_URL, imageUrl.trim());
            }
            String thumbnailImageUrl = getJsonProperty(profile, "thumbnail_image_url");
            if (thumbnailImageUrl != null && !thumbnailImageUrl.isEmpty()) {
                user.setUserAttribute(Constants.FIELD_PROFILE_THUMBNAIL_IMAGE_URL, thumbnailImageUrl.trim());
            }
